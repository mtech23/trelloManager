...
import { faArrowCircleDown, faEdit, faRecycle, faTrash, faTrashAlt, faTrashArrowUp } from "@fortawesome/free-solid-svg-icons";
...
export const WorkPlace = () => {
    ...
    const { ApiData: DeleteBoard, loading: BoardLoading, error: BoardError, del: DeleteBoardCard } = useDelete(`${boardID ? `/api/del-board/${boardID}` : ''}`);
    ...
    const { ApiData: boardUdpatedData, loading: boardUpdateLoading, error: boardUpdateError, post: BoardUpdate } = usePost(`/api/storeboard`);
    ...
    return (
        <DashboardLayout>
                                        ...
                                        {workspace && workspace.map((item, index) => (
                                            ...
                                            <div className="col-12 workspaceArea">
                                                <SkeletonTheme baseColor="grey" highlightColor="silver" key={index}>
                                                    ...
                                                    <div className={`shadow bgWorkCard p-3 rounded-4 mb-4 ${location.pathname.includes(`/${item?.code}`) ? 'activeWspace' : ''}`} key={index}>
                                                        {isLoading ? (
                                                            <>
                        ...
                        <div className="row">
                            {boardData?.boards && boardData?.boards?.map((item, index) => (
                                <div className="col-md-6 mb-4">
                                    <div className={`shadow bgWorkCard p-3 rounded-4`} key={index}>
                                        <div className="boardItem">
                                            <Link
                                                to={`/b/${item?.code}`}
                                                className="nav-link"
                                            ...
                                            <button type="button" className="border-0 text-dark rounded-4 editBtnData" onClick={() => { openBoard(item?.title, item?.id) }}><FontAwesomeIcon icon={faEdit}></FontAwesomeIcon></button>
                                            <button type="button" className="border-0 text-danger rounded-4" onClick={() => { deleteBoardCard(item?.id) }}><FontAwesomeIcon icon={faTrashCan}></FontAwesomeIcon></button>
                                        </div>
                                    </div>
                                </div>
            ...
            <CustomModal show={showUpdate} close={() => { setShowUpdate(false) }} heading='Edit Board' handleSubmit={handleUpdateBoard}>
                ...
                <CustomButton variant="primaryButton" text="Update" type="submit"></CustomButton>
                <CustomModal show={showModal2} close={() => { setShowModal2(false) }} success heading={message} />
